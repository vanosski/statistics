...
export const KINGDOM_CONFIGS: Record<string, KingdomRankingConfig> = {
  ...
  'K48': { redSkills: 4, tierBadge: 'TIER A', badgeClass: 'bg-blue', customBuff: 0.05 },
  ...
  'K166': { redSkills: 3, tierBadge: 'TIER S++', badgeClass: 'bg-crimson' },
...
export function getRankedKingdoms(kingdoms: KingdomSummary[], players: Player[]): RankedKingdom[] {
  const calculated = kingdoms.filter(kd => kd.server !== 'K336').map((kd) => {
